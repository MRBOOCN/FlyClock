...
class FlyClockInstance extends InstanceBase {
	isInitialized = false
	...
	async configUpdated(config) {
		...
		if (oldconfig['url'] !== config['url']) this.initFlyClock()
	}
	...
	initFlyClock() {
		...
		this.ws.on('message', this.messageReceivedFromFlyClock.bind(this))

		this.ws.on('error', (data) => {
			this.log('error', `FlyClock error: ${data}`)
		})
	}

	messageReceivedFromFlyClock(data) {
	...
	getConfigFields() {
		return [
			...
			},
			{
				...
				id: 'url',
				label: 'Target URL',
				tooltip: 'The URL of the FlyClock server (ws[s]://domain[:port][/path])',
				width: 12,
				regex: '/' + this.wsRegex + '/',
			},
			{
				...
				id: 'reconnect',
				label: 'Reconnect',
				tooltip: 'Reconnect on FlyClock error (after 5 secs)',
				width: 6,
				default: true,
			},
			{
			...
			},
			{
				...
				tooltip: 'This prefix will be added to all feedbacks "Update variable with value from FlyClock message" in front of the JSON path option.',
				default: '',
				width: 6,
				regex: '/^[\\w\\.\\-_+\\/\\\\\\$ ]*$/',
			},
			{
				...
				tooltip: 'This suffix will be added to all feedbacks "Update variable with value from FlyClock message" after the JSON path option.',
				default: '',
				width: 6,
				regex: '/^[\\w\\.\\-_+\\/\\\\\\$ ]*$/',
			},
	...
	initFeedbacks() {
		this.setFeedbackDefinitions({
			flyclock_variable: {
				type: 'advanced',
				name: 'Update variable with value from FlyClock message',
				description:
					'Receive messages from the FlyClock and set the value to a variable. Variables can be used on any button.',
				options: [
					{
						type: 'textinput',
...
export { FlyClockInstance as Instance, runEntrypoint }
